    ...
    private:
        template <typename T> void generic_update(std::span<T> data) noexcept
        {
            ...
            for (const auto &i : data)
            {
                ...
                if (m_blocklen == SHA256_BLOCK_SIZE)
                {
                    transform();
        ...
        void transform()   // Process the message in successive 512-bit chunks
        {
            ...
            for (uint32_t i = 0, j = 0; i < 16; i++, j += 4)
                w[i] = static_cast<uint32_t>((m_data[j] << 24u) | (m_data[j + 1u] << 16u) | (m_data[j + 2u] << 8u)
                                             | (m_data[j + 3u]));

            // Extend the first 16 words into the remaining 48 words w[16..63] of the message schedule array
            for (uint8_t k = 16; k < SHA256_ROUNDS; k++)
            {
                uint32_t s0 = sha256::sig0(w[k - 15]);
                uint32_t s1 = sha256::sig1(w[k - 2]);
            ...
            for (uint8_t i = 0; i < SHA256_ROUNDS; i++)
            {
        ...
        std::array<uint8_t, SHA256_BLOCK_SIZE>                   m_data;
